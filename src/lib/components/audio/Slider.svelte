<script lang="ts">
    ...
    $: percentOf = function (value: number) {
    ...
    $: clampValue = function (value: number) {
        return value <= min ? min : value >= max ? max : value;
    };

    $: alignValueToStep = function (value: number) {
        if (value <= min) {
            return min;
        } else if (value >= max) {
            return max;
    ...
    function index(element: HTMLElement ) {
        if (!element) return -1;
        var i = 0;
    ...
    function handleInteract(clientPosition) {
        ...
        if (vertical) {
            ...
            interactionPercent =
                (interactionPosition / dimensions.height) * 100;
    ...
    function bodyMouseUp(event) {
        ...
        if (
            handleActivated &&
            (element === slider || slider.contains(element))
        ) {
            focus = true;
